...
async function generateSchema(event) {
  ...
  const converseCommand = new ConverseCommand({
    ...
    system: [
      {
        text: "You are an expert recipe extractor that compiles recipe details.",
      },
    ],
    messages: [
      {
        ...
        content: [
          {
            text: `Language used in the recipe: ${language}.
  ...
  recipeSchema.llmCost = { inputTokens, outputTokens };
